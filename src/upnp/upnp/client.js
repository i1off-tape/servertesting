...
class Client {
    ...
    constructor(opts = {}) {
        ...
        this.ssdp = new ssdp_1.default({ permanentFallback: this.permanentFallback });
        this.timeout = 3600;
    ...
    async findGateway() {
        if (this._destroyed)
            throw new Error('client is destroyed');
        const { device, address } = await this.ssdp.search('urn:schemas-upnp-org:device:InternetGatewayDevice:1', this.timeout);
