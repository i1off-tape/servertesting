...
class WebSocketHandler {
    ...
    wsOnConnection(ws, req) {
        ...
        ws.on("close", (code, reason) => this.processClose(state, sessionID));
    ...
    processMessageString(state, msg, msgStr) {
    ...
    processObject(state, jsonObject) {
            ...
            if (connect) {
                if (!match.PlayerJoined(profileId)) {
                    ...
                    if (ws.readyState === ws_1.default.OPEN) {
                        ws.send(JSON.stringify({ "endSession": true, reason: CoopMatch_1.CoopMatchEndSessionMessages.WEBSOCKET_TIMEOUT_MESSAGE }));
                        ws.close();
                    }
    ...
    sendToAllWebSockets(data, dataString) {
        this.sendToWebSockets(Object.keys(this.webSockets), data, dataString);
    }
    sendToWebSockets(sessions, data, dataString) {
